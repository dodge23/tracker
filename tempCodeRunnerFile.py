...
def get_ai_response(input_text):
    ...
    url = "https://huggingface.co/chat/models/meta-llama/Meta-Llama-3.1-70B-Instruct"  # Replace with your model's URL
    payload = {"inputs": input_text}

    response = requests.post(url, headers=headers, json=payload)
    if response.status_code == 200:
        return response.json()  # Modify this according to the expected response format
    else:
